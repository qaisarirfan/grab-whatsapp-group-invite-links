...
`src/popup/index.tsx` is a thin mount wrapper — it just calls `createRoot` and renders `<App />`. All state, orchestration, and rendering lives in `src/components/App.tsx`.
...
Service worker. Stateless — no business logic beyond install/update lifecycle.
...
Root component rendered by `src/popup/index.tsx`. It owns the top-level state below and wires the hooks above together.
