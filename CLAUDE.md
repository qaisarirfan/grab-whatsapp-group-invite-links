...
There is no test suite or lint script defined; `npm test` will fail.
...
4. For Google search mode, `fetchAll()` uses `axios` with `Bottleneck` rate limiting (50 concurrent, 200ms between requests) and `p-limit` (concurrency cap of 50) to scrape each search result. `cheerio` parses the HTML server-side to extract WhatsApp links.
5. After links are found, users can validate them via `validateMultipleLinks()` in `src/validation.ts`, which does a `no-cors` HEAD request per link and caches results in `chrome.storage.local` for 24 hours.
